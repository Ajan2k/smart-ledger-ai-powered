...
export default function Setting({ user, open, onOpenChange }: UserProps) {
    ...
    const addTag = () => {
        ...
        if (value && !state.tags.some(tag => tag.name === value)) {
            setState(prev => ({
                ...prev,
                tags: [...prev.tags, {
                    id: `tag_${Math.random().toString(36).substring(2, 15)}`,
                    name: value,
        ...
        if (value && !state.locations.some(loc => loc.name === value)) {
            setState(prev => ({
                ...
                locations: [...prev.locations, {
                    id: `loc_${Math.random().toString(36).substring(2, 15)}`,
                    name: value,
                    createdAt: new Date().toISOString()
                }]
            }))
    ...
    const handleCategoryEdit = (catId: string, newName: string, newColor?: string) => {
        const updatedCategories = (state.categories || []).map(c =>
            c.id === catId
                ? { ...c, name: newName, color: newColor || c.color }
                : c
        );
        setState(prev => ({
            ...prev,
    ...
    const addCategory = () => {
        ...
        if (value && !(state.categories || []).some(cat => cat.name === value && cat.type === newCategoryType)) {
            setState(prev => ({
                ...prev,
                categories: [...(prev.categories || []), {
                    id: `cat_${Math.random().toString(36).substring(2, 15)}`,
                    name: value,
                    type: newCategoryType,
                    color: selectedCategoryColor || TAG_COLORS[0].value,
                    createdAt: new Date().toISOString()
                }]
            }))
            setNewCategoryName('')
                            ...
                            <div className="flex flex-wrap gap-2">
                                {(state.categories || []).map(category => (
                                    <React.Fragment key={category.id}>
                                        <Button
                                            variant="outline"
                                            className="rounded-full min-w-fit flex items-center gap-1.5 h-9"
                                            onClick={() => {
                                                setSelectedCategoryColor(category.color || TAG_COLORS[0].value);
                                        ...
                                        >
                                            ...
                                            <span className="text-[10px] text-gray-400 capitalize">({category.type})</span>
                                        </Button>
                                        <Drawer open={openCategoryId === category.id} onOpenChange={(open) => !open && setOpenCategoryId(null)} repositionInputs={false}>
                                            <DrawerContent>
                                                <DrawerHeader>
                                                        ...
                                                        <div className="flex flex-wrap gap-2">
                                                            {TAG_COLORS.map(color => (
                                                                <button
                                                                    ...
                                                                    className={cn(
                                                                        "w-8 h-8 rounded-full border-2 transition-all",
                                                                        category.color === color.value ? "border-foreground" : "border-transparent"
                                                                    )}
                                                                    style={{ backgroundColor: color.value }}
                                                                    onClick={(e) => {
                                                                        e.preventDefault();
                                                                        handleCategoryEdit(category.id, category.name, color.value);
                                                                    }}
                                                                />
                                                            ))}
                                                        </div>
                                ...
                                <Button
                                    ...
                                    size="icon"
                                    onClick={() => {
                                        setSelectedCategoryColor(TAG_COLORS[0].value);
                                        setNewCategoryType('expense');
                                        setOpenCategoryId('new');
                                    }}
                                    className="rounded-full h-9 w-9"
                                >
                                    <Plus className="h-4 w-4" />
                                </Button>
                                <Drawer open={openCategoryId === 'new'} onOpenChange={(open) => !open && setOpenCategoryId(null)} repositionInputs={false}>
                                    <DrawerContent>
                                        <DrawerHeader>
                                                ...
                                                <div className="flex flex-wrap gap-2">
                                                    {TAG_COLORS.map(color => (
                                                        <button
                                                            ...
                                                            className={cn(
                                                                "w-8 h-8 rounded-full border-2 transition-all",
                                                                selectedCategoryColor === color.value ? "border-foreground" : "border-transparent"
                                                            )}
                                                            style={{ backgroundColor: color.value }}
                                                            onClick={(e) => {
                                                                e.preventDefault();
                                                                setSelectedCategoryColor(color.value);
                                                            }}
                                                        />
                                                    ))}
                                                </div>
