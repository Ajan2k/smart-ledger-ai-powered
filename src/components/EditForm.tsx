...
export default function EditForm({
  ...
  const filteredIncomeCategories = userCategories.filter(c => c.type === 'income');
  const filteredExpenseCategories = userCategories.filter(c => c.type === 'expense');
  const filteredCategories = formData.type === 'income' ? filteredIncomeCategories : filteredExpenseCategories;
      ...
      <CardHeader>
        <div className="flex justify-between items-center">
          <CardTitle className="flex items-center gap-2">
            ...
            <Dialog>
              <DialogTrigger asChild>
                <span className="cursor-pointer hover:bg-gray-100 py-1 px-2 rounded-md max-w-50 min-w-30 text-left border border-gray-300 truncate flex items-center gap-1.5 h-9">
                  {formData.category ? (
                    <>
                      <span 
                        className="w-2.5 h-2.5 rounded-full shrink-0" 
                        style={{ 
                          backgroundColor: filteredCategories.find(c => c.name === formData.category)?.color || '#9ca3af' 
                        }}
                      />
              ...
              <DialogContent className="max-h-[85dvh] flex flex-col">
                <DialogHeader>
                  <DialogTitle>Select Category ({formData.type})</DialogTitle>
                </DialogHeader>
                ...
                <div className="p-4 space-y-4">
                  ...
                  <div className="flex gap-2">
                    <Button
                      ...
                      onClick={() => {
                        const firstCat = filteredIncomeCategories[0]?.name || '';
                        handleChange({ type: 'income', category: firstCat });
                      }}
                      className="flex-1"
                    >
                      Income
                    ...
                    <Button
                      ...
                      onClick={() => {
                        const firstCat = filteredExpenseCategories[0]?.name || '';
                        handleChange({ type: 'expense', category: firstCat });
                      }}
                      className="flex-1"
                    >
                      Expense
